...
def extract_pagination(filters, model):

    # if not get(filters, ['offset']):
    #     filters['offset'] = get(filters, ['page'])

    paginator_data = {
        'limit': get(filters, ['limit']),
        'offset': get(filters, ['offset']),
        'page': get(filters, ['page']),
        'sort': get(filters, ['sort'])
    }

    filters = remove_key_from_dict(filters, 'limit')
    filters = remove_key_from_dict(filters, 'offset')
    filters = remove_key_from_dict(filters, 'page')
    filters = remove_key_from_dict(filters, 'single')
    filters = remove_key_from_dict(filters, 'paginator')
    filters = remove_key_from_dict(filters, 'sub_categories')
    filters = remove_key_from_dict(filters, 'filter_page')
    filters = remove_key_from_dict(filters, 'filter_price')
    filters = remove_key_from_dict(filters, 'filter_brand')
    filters = remove_key_from_dict(filters, 'filter_discount')
    filters = remove_key_from_dict(filters, 'sort')
    filters = remove_key_from_dict(filters, 'sub')
    filters = remove_key_from_dict(filters, 'is_like')
    filters = remove_key_from_dict(filters, 'attribute_like')
    filters = remove_key_from_dict(filters, 'query')
    filters = remove_key_from_dict(filters, 'structure')
    filters = remove_key_from_dict(filters, 'validate_coupon')
    filters = remove_key_from_dict(filters, 'user_logged_id')
    filters = remove_key_from_dict(filters, 'verification_id')
    filters = remove_key_from_dict(filters, 'random')
    filters = remove_key_from_dict(filters, 'action')
    filters = remove_key_from_dict(filters, 'stock_product_id')

    status = 'ACTIVE'
    if get(filters, ['status']):
        status = get(filters, ['status'])

    if hasattr(model, 'status') and status != 'BOTH':
        # if issubclass(model, Order) and status == 'ACTIVE':
        #     status = 'PENDING'
        filters['status'] = status
    else:
        filters = remove_key_from_dict(filters, 'status')

    return paginator_data, filters
